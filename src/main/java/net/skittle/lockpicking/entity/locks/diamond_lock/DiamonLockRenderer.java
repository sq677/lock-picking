...
public class DiamonLockRenderer extends EntityRenderer<DiamondLockEntity> {
    public static final EntityModelLayer LAYER = new EntityModelLayer(Identifier.of("lockpicking", "diamond_lock"), "main");
    private static final Identifier TEXTURE = Identifier.of("lockpicking", "textures/entity/diamond_lock.png");
    private final DiamondLockModel model;

    public DiamonLockRenderer(EntityRendererFactory.Context ctx) {
        super(ctx);
        this.model = new DiamondLockModel(ctx.getPart(LAYER));
    }


    @Override
    public void render(DiamondLockEntity entity, float yaw, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light) {
        float lidAngle = 0f;
        ...
        matrices.translate(0.1, -1.4, 0.001);
    ...
    @Override
    public Identifier getTexture(DiamondLockEntity entity) {
        return TEXTURE;
    }
}
